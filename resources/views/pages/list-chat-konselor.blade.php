                            ...
                            <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle mt-2" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $item->orangtua->orangtua->nama }}</td>
                                            <td>{{ $item->konselor->konselor->nama_konselor }}</td>
                                            <td>
                                                <a href="{{ route('start-chat-konselor', [$item->id]) }}" class="btn btn-success">Mulai Chat</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <div class="modal fade modalKonselor" tabindex="-1" aria-labelledby="exampleModalFullscreenLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <form action="{{ route('add-chat-konselor') }}" method="POST" id="formKonselor">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalFullscreenLabel">Tambah Chat Konselor</h5>
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-control" name="konselor_id" required>
                                    ...
                                    @foreach ($konselor as $value)
                                        <option value="{{ $value->id }}">{{ $value->nama_konselor }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
